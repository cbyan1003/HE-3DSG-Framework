...
//lyc add
	...
	void setFps(int f) { fps->Ref().Set(f);}//Set(f);}
...
//lyc add over
	...
	std::vector<int> bbx_instanceTable_idx;
	std::vector<std::vector<float>> bbx_centroid;
	std::vector<std::pair<std::vector<float>,std::vector<float>>> bbx_minmax;
	std::set<float> xyz;
