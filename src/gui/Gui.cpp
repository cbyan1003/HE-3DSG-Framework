	...
	, panel_(205)
{
	...
	pangolin::CreateWindowAndBind("InstanceFusion", width_, height_, window_params);//启动窗口

	std::vector<std::set<float>> bbx_centroid;

	//Main Display
	cout<< "开始初始化gui " <<endl;
	// CUDA 和 OpenGL的互相映射，用OPENGL渲染图像，用cuda进行计算
	// OpenGL与CUDA互操作可以分成两种，
	// 一种是OpenGL将Buffer对象注册到CUDA中去，供CUDA读写操作，然后再在OpenGL中使用。一般这种情况下注册的是VBO和PBO，VBO一般用于存储顶点坐标、索引等数据；PBO则一般用于存储图像数据，因此称作Pixel Buffer Object。
	// 另一种是OpenGL将Texture对象注册到CUDA中去，经CUDA处理后得到纹理内容，然后在OpenGL中渲染出来。不过不管是哪一种互操作类型，其操作流程是一致的：
	...
	glViewport(0, 0, mainWidth, mainHeight);//指定窗口，（0，0）左下角点
	...
	s_cam_ = pangolin::OpenGlRenderState(pangolin::ProjectionMatrix(640, 480, 420, 420, 320, 240, 0.1, 1000),/*图像长度、宽度、4个内参ux uy vx vy以及最近最远视距//类似内参*/
										pangolin::ModelViewLookAt(0, 0, -1, 0, 0, 1, pangolin::AxisNegY));/*相机位置，相机所看视点位置，相机轴方向//类似外参*/
	pangolin::Display("cam").SetBounds(0, 1.0f, 0, 1.0f, -640 / 480.0)/*底、顶、左、右、显示长宽比//视图在视窗中的范围*/
				.SetHandler(new pangolin::Handler3D(s_cam_));/*handler绑定渲染状态*/
				
	// Small views along the bottom
	pangolin::Display("raw").SetAspect(640.0f/480.0f);//Add named OpenGL viewport to window 
	//lyc add
	...
	instance_view_.reset(new pangolin::Var<bool>("ui.Draw Instances", true, false));//
	time_view_.reset(new pangolin::Var<bool>("ui.Draw Time", true, false));//
	projectBoudingBox_.reset(new pangolin::Var<bool>("ui.Draw ProjectMap BBox", true, false));//
	...
	if(drawBox&&n>0)
	{
		...
		for(int i=0;i<instanceNum;i++)
		{
			...
			*/
			...
			classid = instanceFusion->getInstanceClass(i);
			//Debug
			...
			bbx_instanceTable_idx.emplace_back(classid);
			min.emplace_back(minX);
			min.emplace_back(minY);
			min.emplace_back(minZ);
			max.emplace_back(maxX);
			max.emplace_back(maxY);
			max.emplace_back(maxZ);
			minmax.first = min;
			minmax.second = max;
			bbx_minmax.emplace_back(minmax);

			min.clear();
			max.clear();
			
			// xyz.insert(minmax);
			// bbx_minmax.emplace_back(xyz);
			...
			for(int j = 0 ;j < 8;j++)
			{
				x += boxPoint3d_world[j][0];
				y += boxPoint3d_world[j][1];
				z += boxPoint3d_world[j][2];
			}
			x = x/8;
			y = y/8;
			z = z/8;
			...
			xyz.emplace_back(y);
			xyz.emplace_back(z);
			bbx_centroid.emplace_back(xyz);
			xyz.clear();
			std::cout<<std::endl;
			// std::cout<<std::endl;
			// std::cout<<"=============debug==============="<<std::endl;
			// std::cout<<"bbx_minmax "<<std::endl;
			// for(auto n:bbx_minmax)
			// {
			// 	for(auto nn:n.first)
			// 	{
			// 		std::cout<<nn<<" ";
			...
			// }
			// std::cout<<std::endl;
			// std::cout<<"bbx_centroid "<<std::endl;
			// for(auto n:bbx_centroid)
			// {
			// 	for(auto nn:n)
			// 	{
			// 		std::cout<<nn<<" ";
			...
			// }
			// std::cout<<std::endl;
			// std::cout<<"bbx_instanceTable_idx "<<std::endl;
			// for(auto n:bbx_instanceTable_idx)
			// {
			// 	std::cout<<n<<" ";
			// }
			// std::cout<<std::endl;
