...
#include <cstring>
#include "/home/ycb/InstanceFusion/src/graph/JsonUtil.h"
#include <iostream>
    ...
    public:
        ...
        ParamLoader(std::string path, std::string name_args = "args.json"):pth_base(path){
            ...
            std::cout<<"pth_base: "<<pth_base<<std::endl;
            const std::string pth_onnx = pth_base + name_args;
            std::cout<<"pth_onnx: "<<pth_onnx<<std::endl;
            ...
            /// Read labels
            {
                ...
                if(!file.is_open())std::cout << "Cannot open class file at path " << path+"/"+"classes.txt";
                std::string line;
            ...
            }
            {
                ...
                if(!file.is_open())std::cout << "Cannot open relationship file at path " << path+"/"+"relationships.txt";
                std::string line;
                std::vector<std::string> labels;
                while (std::getline(file, line)) labels.push_back(line);
