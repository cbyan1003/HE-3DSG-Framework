...
#ifndef ONNXMODELBASE_H
#define ONNXMODELBASE_H
...
#include<sstream>
    ...
    namespace ONNX {
        ...
        class OnnxModelBase: public ParamLoader {
        public:
        ...
        protected:
            ...
            virtual void InitModel(){
                ...
                for(auto &pair:mModelParams) {
                    ...
                    //对于每一个model创建一个session,通过run方法来进行模型推理
                }
            }
            ...
            inline std::vector<Ort::Value> Run(const std::string &name, const std::vector<Ort::Value> &input_tensor){
                return mSessions.at(name)->Run(Ort::RunOptions{nullptr},
                                               ...
                                               //mModelParams,map,name是args.json中定义的模型model_XXX,,vector.data()指针
            }
