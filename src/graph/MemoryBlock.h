...
#include "/home/ycb/InstanceFusion/src/gSLICr/ORUtils/Vector.h"
#include <cassert>
#include <cstring>
#include <iostream>
template<typename T>
...
//        SCLOG(VERBOSE) << this << " move constructor called";
    ...
    void operator=(MemoryBlock const &x){ //  copy assignment operator
        if (!this->mData) std::cout << "block was not initialized."<<std::endl;
        if (this->mDtype != x.mDtype) std::cout << "";
        if (this->mSize != x.mSize) std::cout << "size mismatch"<<std::endl;
