...
{
	const int x = blockIdx.x * blockDim.x + threadIdx.x;//全局索引
				...
				const int width, const int height,const int surfel_size,const int surfel_instance_offset)
{
	...
	const int x = blockIdx.x * blockDim.x + threadIdx.x;
	const int y = blockIdx.y * blockDim.y + threadIdx.y;
// blockIdx.x = 0-31 ; blockDim.x = 32 ; threadIdx.x = 0-20 ====> x = 0-1012
// blockIdx.y = 0-31 ; blockDim.y = 32 ; threadIdx.y = 0-15 ====> y = 0-1007
	int surfel_id = tex2D<int>(map_ids,x,y);
	int instanceNum_half = surfel_size - surfel_instance_offset;// surfel_size = 64  surfel_instance_offset = 16
	...
	for(int i=0; i<instanceNum_half; i++)
	{
		if (surfel_id > 0&&surfel_id<n) 
		{
			...
			// printf("mapInfo:%16f\n",mapInfo);
			//decode
			instanceProjectMap[ y*width + x + (2*i)*width*height ] = decode1_Instance(mapInfo);//向右位移16bit，& 0xFFFF 
			// printf("decode1_Instance(mapInfo):%20f\n",decode1_Instance(mapInfo));
			instanceProjectMap[ y*width + x + (2*i+1)*width*height ] = decode2_Instance(mapInfo);
			// printf("decode2_Instance(mapInfo):%20f\n",decode2_Instance(mapInfo));
		}
		else
		{
...
{
	const int index = blockIdx.x * blockDim.x + threadIdx.x;//全局索引
	int id = index/4;
	// printf("id:%d",id);
...
void computeProjectBoundingBoxKernel(unsigned char* masks, const int masksNum, int* instanceProjectMap, const int width, const int height,
								 const int instanceNum, int* MasksBBox, int* projectBBox)//width = 640 height = 480
// Grid > Block(32,32,1) > Thread(20,15,1)
// test learn CUDA  
// {
// 	const int x = blockIdx.x * blockDim.x + threadIdx.x;// blockIdx.x = 0-31 ; blockDim.x = 32 ; threadIdx.x = 0-20 ====> x = 0-1012
// 	const int y = blockIdx.y * blockDim.y + threadIdx.y;// blockIdx.y = 0-31 ; blockDim.y = 32 ; threadIdx.y = 0-15 ====> y = 0-1007
// 	for(int instanceID=0;instanceID<instanceNum;instanceID++)
// 	{
// 		if(instanceProjectMap[ y*width + x] > 0)
// 		printf("instanceProjectMap:%d y*width + x = %d\n",instanceProjectMap[ y*width + x],y*width + x);
...

{
	...
	if(instanceProjectMap[ y*width + x]!=-1)
	{
		...
		for(int instanceID=0;instanceID<instanceNum;instanceID++)
		{
			int* instance = instanceProjectMap + instanceID*width*height;//instanceProjectMap为指针指向的地址
			// printf("instance[]:%d\t",instance[ y*width + x]);
			// printf("y*width + x = %d\n",y*width + x);
			// printf("y + x*height = %d\n",y + x*height);
						 ...
						 const int instanceNum, int* MasksBBox, int* projectBBox)
{
	...
	const int blocks1 = ((instanceNum + masksNum)*4 + threads1 - 1) / threads1;//(96+masks)*4+511
	...
	dim3 dimGrid2(blocks2,blocks2);//(32,32)
	dim3 dimBlock2(width/blocks2,height/blocks2);//(20,15)
	
	// dim3 Grid1(3,3);
	// dim3 Block1(3,3);
	// computeProjectBoundingBoxKernel<<<Grid1,Block1>>>(masks, masksNum, instanceProjectMap, width, height, instanceNum, MasksBBox, projectBBox);
									...
									const int surfel_instanceColor_offset, float* instanceTable_color, int* bestIDInEachSurfel)
{
	...
	if (id < n) {
		int BestInstance = -1;
		int BestCount = 0;
		int instanceNum_half = surfel_size - surfel_instance_offset;//64-16=48
		
		for(int i=0; i< instanceNum_half; i++)
		{
			float mapInfo = map_surfels[id * surfel_size + surfel_instance_offset + i];
			// surfel siez 64 + normal 8 + instance 16 + rgb 4 + instance color 5 + instance GT 15
			...
			if(BestCount < instanceI_1)
			{
				...
				BestInstance = 2 * i;
				// printf("instanceI_1:%d , BestInstance:%d , i:%d\n",instanceI_1,BestInstance,i);
			}
			if(BestCount < instanceI_2)
			{
				BestCount = instanceI_2;
				BestInstance = 2 * i + 1;
				// printf("instanceI_2:%d , BestInstance:%d , i:%d\n",instanceI_2,BestInstance,i);
			}
		}
		...
		if(map_surfels[id*surfel_size+surfel_instanceColor_offset]==0||map_surfels[id*surfel_size+surfel_instanceColor_offset]==defaultColor)
		{
			if(BestInstance != -1)
			{
				map_surfels[id * surfel_size + surfel_instanceColor_offset] = instanceTable_color[BestInstance];//之前位运算，压缩的颜色，24bit (r_8bit g_8bit b_8bit),float类型有32bit
			}
			else
			{
				map_surfels[id * surfel_size + surfel_instanceColor_offset] = defaultColor;
			}
			//map_surfels 结构：id*surfel size = 64bit + 5 bit + 
		}
...
void renderMaskFrame(unsigned char* masks, const int masksNum, const int width, const int height, float* maskColor)
{
	...
	// Dim3 Dg(Dg.x, Dg.y, 1)表示grid中每行有Dg.x个block，每列有Dg.y个block，第三维恒为1(目前一个核函数只有一个grid)。整个grid中共有Dg.x*Dg.y个block，其中Dg.x和Dg.y最大值为65535。
	// 参数Db用于定义一个block的维度和尺寸，即一个block有多少个thread。为dim3类型。
	// Dim3 Db(Db.x, Db.y, Db.z)表示整个block中每行有Db.x个thread，每列有Db.y个thread，高度为Db.z。Db.x和Db.y最大值为512，Db.z最大值为62。 一个block中共有Db.x*Db.y*Db.z个thread。计算能力为1.0,1.1的硬件该乘积的最大值为768，计算能力为1.2,1.3的硬件支持的最大值为1024。
	gpuErrChk(cudaGetLastError());
	gpuErrChk(cudaDeviceSynchronize());
}
								...
								float* instanceTable_color, const int surfel_instanceColor_offset, int* eachInstanceSumCount)
{
	...
	const int instanceNum_half = surfel_size - surfel_instance_offset;
	const int instanceNum      = instanceNum_half*2;
	// printf("surfel_size %d surfel_instance_offset %d surfel_size %d surfel_instanceColor_offset %d\n",&surfel_size,&surfel_instance_offset,&instanceNum,&surfel_instanceColor_offset);
	if (id < n) {
		// printf("id %d instanceNum %d",&id,&instanceNum);
		int instance = -1;
		for(int i=0;i<instanceNum;i++)
		{
			if(map_surfels[id*surfel_size+surfel_instanceColor_offset] ==instanceTable_color[i])
