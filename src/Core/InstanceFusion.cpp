...
InstanceFusion::InstanceFusion(int num,int w,int h,bool useMulThread,int netType)//构造函数，在对象创建时执行
{
	//construct
	instanceNum = num;
	width = w;
	...
	//InstanceFusion
	instanceInit();
	cout << "instanceFuison C++ init ok!" << endl;
	//MaskRcnn
	if(!useThread){
		cout << "dont use thread!" << endl;
		pythonInit();		//PlanA  synchronous//引用python文件，python函数
...
InstanceFusion::~InstanceFusion()//析构函数，在删除所创建的对象时执行，释放资源
{
	//释放python
...
void InstanceFusion::pythonInit()
{
	...
	else if(maskrcnnType==1)	//ori maskrcnn
	{
		...
		wchar_t const * argv2[] = { L"/home/ycb/InstanceFusion/build.2/mask_ori.py" };
		PySys_SetArgv(1, const_cast<wchar_t**>(argv2));

		// Load module
		std::cout << " * Loading module..." << std::endl;
		pModule = PyImport_ImportModule("mask_ori");//引用python模块，类似import
		if(pModule == NULL) {
			if(PyErr_Occurred()) {
				std::cout << "Python error indicator is set:" << std::endl;
				PyErr_Print();
	...
	// Get function
	std::cout << "Get function detect..." << std::endl;
	pExecute = PyObject_GetAttrString(pModule, "detect");//返回引用模块中的detect函数
	if(pExecute == NULL || !PyCallable_Check(pExecute)) {//找到名为detect的函数
		if(PyErr_Occurred()) {
			std::cout << "Python error indicator is set:" << std::endl;
			PyErr_Print();
		}
...
bool InstanceFusion::whetherDoSegmentation(const std::unique_ptr<ElasticFusionInterface>& map,int frame_num)
{
	...
	checkProjectDepthAndInstance(index_surfelsIds,n,map_surfels,width,height,surfel_size,surfel_instance_offset,downsample,count_gpu);//cuda编程计算
	...
	// cout << "lastSegFrameID :" << lastSegFrameID <<endl;
	if(test1||test2) 
	{
											...
											int frame_num,bool isflann)
{
	...
	if(!useThread)
	{	
		// cout << "start to detect from python" << endl;
		// TimeTick();
		detect();
		// TimeTock("Detect",true);
		// cout << "start to get mask from python" << endl;
		getCurrentResults();	//从python中读取到mask和相应的classid
		renderRawMask();
		// cout << "renderRawMask ok" << endl;
		processInstance(map,isflann);//论文中图三的两次校正的分割过程
		// cout << "processInstance ok" << endl;
...
void InstanceFusion::detect()
{
	...
	cv::imwrite("/home/ycb/InstanceFusion/build/rgb.png",rgbImage);
	//C++调用python函数、方法等调用都可以通过PyObject_Call系列函数完成
	// cout << "##开始调用detect函数##" <<endl;
	Py_XDECREF(PyObject_CallFunctionObjArgs(pExecute, createArguments(rgbImage),NULL));//调用mask_ori的detect函数，之后立刻释放
	//此处完成了Maskrcnn的第一次分割,同样也是论文中的a环节
	if(PyErr_Occurred()) 
	{
		std::cout << "Python error indicator is set:" << std::endl;
		PyErr_Print();
...
{
	extractMask();//从python的结果中读取mask，存储为一个长字符串
	// cout<<"get mask from python ok"<<endl;
	extractClassIDs();//从python中读取相应mask的classid，数组保存
	// cout<<"get classid from python ok"<<endl;
	if(resultMasks_Num)
	{
...
{
	PyObject* pMasksList = PyObject_GetAttrString(pModule, "masks");//获取python中的masks对象
	...
	resultMasks_Num = PySequence_Length(pMasksList);//返回序列中的对象个数
	// cout << "resultMasks_Num" << resultMasks_Num << endl;
	if(resultMasks_Num) resultMasks = new unsigned char[resultMasks_Num*width*height];//全部mask内容长度的字符串
	else resultMasks = 0;
	...
	{	
		...
		//error? check?
		int h = PyArray_DIM(pMaskArray,0);//PyArray_DIM返回数组的尺寸
		...
		if(width == w && height == h)
		{
			unsigned char* pData = (unsigned char*)PyArray_GETPTR1(pMaskArray,0);//在给定坐标情况下快速访问array中的元素
			memcpy(resultMasks+i*width*height,pData,h*w);//复制（目标，源，复制的字节数）//在字符串后续上一个mask的内容
		}
		Py_DECREF(o);//减少对象的引用计数，如果引用计数降低至0，将会发起调用对象所属类型的释放函数
	}

	if(resultMasks_Num)
	{
	 	cudaMalloc((void **)&resultMasks_gpu, resultMasks_Num*width*height * sizeof(unsigned char));//第一个参数传递的是存储在cpu内存中的指针变量的地址，
	...
	Py_DECREF(pMasksList);//减少对象的引用计数，如果引用计数降低至0，将会发起调用对象所属类型的释放函数
	return;
...
void InstanceFusion::extractClassIDs()
{
	...
	for (int i = 0; i < resultMasks_Num; i++) 
	{
		...
		// cout << "class_ids " << resultClass_ids[i] << endl;
		Py_DECREF(o);
	}
...
{
	assert(rgbImage.channels() == 3);//C++的断言，检测图片是否为三通道
	npy_intp dims[3] = { rgbImage.rows, rgbImage.cols, 3 };
	import_array();//初始化 numpy 执行环境，python2.7用void返回类型，python3.0以上用int返回类型
	// cout << "detect image check over\ntrans img to numpy" <<endl;
	return PyArray_SimpleNewFromData(3, dims, NPY_UINT8, rgbImage.data); //将c的数据转换成numpy
}
//====================InstanceFusion==================================================
...
void InstanceFusion::instanceInit()
{
	...
	createInstanceTable();//创建instanceTable数组存储类名和RGB数值，instanceTable_colorvector存储位运算后拼接成一维的的RGB数值
	cleanTimes = 0;
	...
	// gSLICr 一个用 C++ 和 CUDA 编写的用于实时超像素分割的库
	// gSLICr settings
		...
		for(int instanceID = 0;instanceID<instanceNum;instanceID++)
		{
			if(instTableClassList[instanceID] != -1)	//instance is exist
			{
				if(resultClass_ids[maskID] == instTableClassList[instanceID])	//InstanceClass == MaskClass
				{
					...
					//Debug
					std::cout<<"->instanceID: "<<instanceID<<"  "<<instanceTable[instanceID].name<<"\tmin("<<minX_i<<","<<minY_i<<") max("<<maxX_i<<","<<maxY_i<<")\n";
...
void InstanceFusion::processInstance(const std::unique_ptr<ElasticFusionInterface>& map,bool isflann)
{
	...
	// cout << "get Map Surfel Count : " << n << endl;
	//================init in Each frame=========================
	if(projectBBox) delete[] projectBBox;
	projectBBox = 0;
	if(map3DBBox) delete[] map3DBBox;
	map3DBBox = 0;
	// cout<<"instcMatrix的内存地址:"<<instcMatrix<<endl;
	memset(instcMatrix,0, 16 * instanceNum * sizeof(float));//16*96(*4)
	//void *memset(*s, ch, n);     
	//s是内存地址
    //ch是指定值，将该段内存的所有数值修改为ch
    //n是内存长度
	// cout<<"gcMatrix的内存地址:"<<gcMatrix<<endl;
	...
	for(int i=0;i<instanceNum;i++)
	{
		...
		//一个instance的存储长度为16位，首位是1，中间5和10是-1,共96个instance
	}
	for(int i=0;i<instanceNum;i++)
	{
		if(instSurfels[i]) 
			// cout<<"instSurfels["<<i<<"]:"<<instSurfels[i]<<endl;
			// delete instSurfels[i];
		instSurfels[i]=0;
	}
	//===========================================================
	// cout << "here" <<endl;
	...
	// cout<< "start to do SuperPixels" << endl;
	//SuperPixels
	// std::cout<<std::endl;
	//step -1_1 gSLICr
	// TimeTick();
	...
	for(int i=0;i<resultMasks_Num;i++)
	{
		...
		std::string savename = "/home/ycb/InstanceFusion/build/temp/mask"+std::to_string(frameID) +"_"+ std::to_string(i) +"_"+std::to_string(resultClass_ids[i])
	...
	// TimeTock("CompareMap");
	// for(int i =0 ; i < resultMasks_Num * instanceNum;i++)
	// {
	// 	cout << "compareMap" << compareMap[i] << endl;	
	// }
	...
	std::string savenameProj = "/home/ycb/InstanceFusion/build/temp/projectDepth.png";
	...
	for(int maskID = 0; maskID < resultMasks_Num; maskID++)
	{
		// std::cout<< "maskID : " << maskID << "  resultMasks_Num : " << resultMasks_Num << "  instanceNum : " << instanceNum << std::endl;
		//Debug
		...
		if(!instanceExist && !unavailableMask[maskID])//如果comparemap没有命中
		{
		...
		for(int instanceID = 0; instanceID < instanceNum; instanceID++)
		{
			if(compareMap[instanceID + maskID * instanceNum] == 1)
			{
				...
				// std::cout<<"step 3_2 updateSurfelMapInstance"<<std::endl;
...
void InstanceFusion::computeMapBoundingBox(const std::unique_ptr<ElasticFusionInterface>& map, bool bboxType)
{
	...
	// std::cout<<"n: "<<n<<" map_surfels "<<map_surfels<<std::endl;
	//Step 1 find Normal
	//TimeTick();
...
void InstanceFusion::getInstancePointCloud(const std::unique_ptr<ElasticFusionInterface>& map,bool bboxType)
{
	...
	int  instTableClassList[instanceNum];									//input
	getInstanceTableClassList(instTableClassList);//instTableClassList的第i个位置存储第i个instanceclassid
	...
	for(int i=0;i<instanceNum;i++) 
	{
		if(eachInstanceSumCount[i])
			// std::cout<<"instance size: "<<eachInstanceSumCount[i]<<std::endl;
		if(instSurfels_cpu2gpu[i]) 
		{
			for(int j=0;j<classNum;j++)
			{
				...
				{
					int rr = instanceTable[i].r;
					int gg = instanceTable[i].g;
					int bb = instanceTable[i].b;
					std::cout<<instanceTable[i].name<<"  "<<rr<<"  "<<gg<<" "<<bb<<std::endl;
					Eigen::Vector4f * mapData = map->elastic_fusion_->getGlobalModel().downloadMap();
					// std::cout<<"lastCount: "<<map->elastic_fusion_->getGlobalModel().lastCount()<<std::endl;
					for(unsigned int k = 0; k < map->elastic_fusion_->getGlobalModel().lastCount(); k++)
					{
						
						Eigen::Vector4f pos = mapData[(k * 16) + 0];
						Eigen::Vector4f col = mapData[(k * 16) + 1];
						Eigen::Vector4f nor = mapData[(k * 16) + 2];
						nor[0] *= -1;
						nor[1] *= -1;
						nor[2] *= -1;
						...
						unsigned int r = int(col[1]) >> 16 & 0xFF;
						unsigned int g = int(col[1]) >> 8 & 0xFF;
						unsigned int b = int(col[1]) & 0xFF;
						// std::cout<<"r:"<<(int(col[1]) >> 16& 0xFF)<<" g:"<<(int(col[1]) >> 8& 0xFF)<<" b:"<<(int(col[1])& 0xFF)<<std::endl;
						if((rr == r)&&(gg == g)&&(bb == b))
						{
							// std::cout<<"嘻嘻命中"<<std::endl;
	...
	for(int i=0;i<instanceNum;i++)
	{
		if(instSurfels_cpu2gpu[i]) 
		{
			...
			// std::cout<<instSurfels_cpu2gpu[i]<<std::endl;
			
			// std::cout<<instSurfels[i][0]<<std::endl;
			// std::cout<<instSurfels[i][1]<<std::endl;
			// std::cout<<instSurfels[i][2]<<std::endl;
			// std::cout<<instSurfels[i][3]<<std::endl;
			// std::cout<<instSurfels[i][4]<<std::endl;
			// std::cout<<instSurfels[i][5]<<std::endl;
			// std::cout<<instSurfels[i][6]<<std::endl;
			// std::cout<<instSurfels[i][7]<<std::endl;
			// std::cout<<instSurfels[i][8]<<std::endl;
			// std::cout<<instSurfels[i][9]<<std::endl;
			// std::cout<<instSurfels[i][10]<<std::endl;
			// std::cout<<instSurfels[i][11]<<std::endl;
			// std::cout<<instSurfels[i][12]<<std::endl;
			// std::cout<<instSurfels[i][13]<<std::endl;
...
void InstanceFusion::getPointCloud(const std::unique_ptr<ElasticFusionInterface>& map,int num)
{
	featuremap.clear();
	colormap.clear();
	for(int i=0;i<num;i++) 
	{
		...
		std::vector<int> inscolor;
		inscolor.push_back(rr);
		inscolor.push_back(gg);
		inscolor.push_back(bb);
		for(unsigned int k = 0; k < map->elastic_fusion_->getGlobalModel().lastCount(); k++)
		{
			Eigen::Vector4f pos = mapData[(k * 16) + 0];
			Eigen::Vector4f col = mapData[(k * 16) + 1];
			...
			if((rr == r)&&(gg == g)&&(bb == b))
			{
				...
				int red = int(col[0]) >> 16 & 0xFF;
				int green = int(col[0]) >> 8 & 0xFF;
				int blue = int(col[0]) & 0xFF;
				// std::cout<<"r: "<<r<<" g: "<<g<<" b: "<<b<<std::endl;
				colorr.push_back(float(red));
				colorr.push_back(float(green));
				colorr.push_back(float(blue));
			}
		}
		std::cout<<"i: "<<i<<" feature.size: "<<poss.size()<<std::endl;
		featuremap.insert({i,poss});
		colormap.insert({i,colorr});
		Instancecolor.insert({i,inscolor});
	...
	// if(num>1)
	// {
	// 	for(auto n:featuremap.at(0))
	// 	{
	// 		std::cout<<n<<" ";
	// 	}
	// }
	std::cout<<"finish get point cloud"<<std::endl;
}

std::vector<float> InstanceFusion::getFeatureMap(int index){
	return featuremap.at(index);	
}

std::vector<int> InstanceFusion::getColorMap(int index){
	return colormap.at(index);	
}

std::vector<int> InstanceFusion::getInstanceColor(int index){
	return Instancecolor.at(index);	
}

void InstanceFusion::saveInstancePointCloud(const std::unique_ptr<ElasticFusionInterface>& map,int num)
{
	//===============================================================
	...
	for(int i=0;i<num;i++) 
	{
		...
		int gg = instanceTable[i].g;
		int bb = instanceTable[i].b;
		std::cout<<"color : "<<rr<<" "<<gg<<" "<<bb<<std::endl;
		std::string filename = "saveFilename_"+to_string(i)+"_test_instance.ply";
		std::ofstream fs;
		int validCount = 0;

		for(unsigned int j = 0; j < map->elastic_fusion_->getGlobalModel().lastCount(); j++)
		{
			Eigen::Vector4f pos = mapData[(j * 16) + 0];

			if(pos[3] > map->elastic_fusion_->getConfidenceThreshold())
			{
				validCount++;
			}
		}
		...
		for(unsigned int k = 0; k < map->elastic_fusion_->getGlobalModel().lastCount(); k++)
		{
			...
			if((rr == r)&&(gg == g)&&(bb == b))
			{
				...
				unsigned int r = int(col[0]) >> 16 & 0xFF;
				unsigned int g = int(col[0]) >> 8 & 0xFF;
				unsigned int b = int(col[0]) & 0xFF;
