...
#include "/home/ycb/InstanceFusion/src/graph/node.h"
#include "/home/ycb/InstanceFusion/src/graph/graph.h"
...
int main(int argc, char *argv[])
{
	...
	// std::cout<<"output: "<<(r<<8) + g<<std::endl;
	// std::cout<<"output: "<<(((r<<8) + g)<<8)<<std::endl;
	// std::cout<<"output: "<<(((r<<8) + g)<<8)+b<<std::endl;


	//Mask R-CNN
	const int cnn_skip_frames = 3;  // 分割间隔cnn_skip_frames帧运行一次
	const int cnn_start_frames = 5;//cnn_start_frames起始帧
	const int maskRcnnType = 1; // maskrcnntype为0时代表使用mask_benchmark，为1时代表使用mask_ori，为2时代表使用mask_paddle。

	// flann frame
	const int flann_skip_frames = 5; // flann(kdtree和三维滤波) 间隔flann_skip_frames帧运行一次
	
	const bool debugMask = true;  // 表示是否输出分割运行时掩码的中间结果
	...
	const int width = 640;//640 // width和height会存在Resolution和Intrinsics中作为全局变量可在任意位置调用。
	...
	else if (argc == 2) // 即使argc的值大于2，只要第一个if执行了，这个else if都不会执行
	{
		...
		cout << log_reader->currentFrame << endl;
		cout << log_reader->getFile() <<endl;
		//cout << log_reader->rgb[1] <<endl;
		cout << log_reader->getNumFrames() << endl;
	} 
	else 
	{
		//kinectv1
	...
	cout << "创建instancefusion对象完成" <<endl;	
	// 创建instancefusion对象,false表示不用多线程
	// 创建对象的同时运行构造函数，进行Instance的instanceInit初始化和python的初始化
	...
	std::cout<<"start to ElasticFusionInterface"<<std::endl;
	...
	while(!pangolin::ShouldQuit() && log_reader->hasMore()) //Pangolin是对OpenGL进行封装的轻量级的OpenGL输入/输出和视频显示的库。可以用于3D视觉和3D导航的视觉图，可以输入各种类型的视频、并且可以保留视频和输入数据用于debug。
	{
		...
		frame_real++;
		// instancefusion->TimeTock("过程0：update GUI");
		//====================lyc add over ============================================================
		...
		{
			// instancefusion->TimeTick();
			// cout << "#######进行重建实例分割过程########" << endl;
			log_reader->getNext();
			//map->setTrackingOnly(gui->tracking());  // TrackingOnly按钮表示界面上的模型是否随着重建更新
			// instancefusion->TimeTock("过程1：读取log的next，设置为tracking only");
			...
			// instancefusion->TimeTock("过程2：从模型中反投影图片中画每个instance的bbx");
			...
			// cout << "gui->instance_seg() :" << gui->instance_seg() << endl;
			// cout << "frame_Fusion :" << frame_Fusion << endl;
			// cout << "cnn_start_frames :" << cnn_start_frames << endl;
			//bool(frame_Fusion >= cnn_start_frames) && gui->instance_seg() && 
			// instancefusion->TimeTick();

			if (frame_Fusion >= cnn_start_frames && gui->instance_seg() && instancefusion->whetherDoSegmentation(map,frame_Fusion))
			// if(bool(frame_Fusion >= cnn_start_frames) && gui->instance_seg())

			// if (frame_Fusion >= cnn_start_frames && gui->instance_seg() && ((frame_Fusion + 1) % cnn_skip_frames == 0))
			{
				// cout << "whetherDoSegmentation :" << instancefusion->whetherDoSegmentation(map,frame_Fusion) << endl;

				bool flannFlag = 0;  // flannFlag是什么？？  FLANN(Fast Library for Approximate Nearest Neighbors)是一个执行快速近似最近邻搜索的库。
				if(frame_Fusion-lastTimeFlann > flann_skip_frames) // frame_Fusion是大循环里面迭代的帧序数
				{
			...
			// instancefusion->TimeTock("过程3：分割的全过程");
			...
			if(gui->raw_save())
			{
				...
				cout<< "rgb_save_dir : " << rgb_save_dir <<endl;
		...
		// }
		// instancefusion->TimeTock("display");
		if(frame_Fusion>50 && gui->display_mode())
		// else	//modify in InstanceFusion(DANGER CODE!JUST USE FOR DEBUG)
		{

			// cout << "display_mode off" <<endl;
			...
			gui->postCall();
			cout<<"===================MAIN DEBUG "<<frame_Fusion<<" ================="<<endl;
			// cout<<"bbx_minmax size : "<<gui->bbx_minmax.size()<<endl;
			int num = gui->bbx_instanceTable_idx.size();
			instancefusion->getPointCloud(map,num);

			graph->Update(gui,instancefusion);
			
			
			// cout<<"bbx_centroid size : "<<gui->bbx_centroid.size()<<endl;
			// for(auto n:gui->bbx_centroid)
			// {
			// 	for(auto nn:n)
			// 	{
			// 		cout << nn << endl;
			...
			if(gui->instance_save_once())
			{
				...
				instancefusion->saveInstancePointCloud(map,num);//not finish  将96个instance分别输出到/temp文件夹下，会按照地面坐标系(或instance坐标系)修正位置，其中地面也会单独输出。
	...
	int num = gui->bbx_instanceTable_idx.size();
	instancefusion->saveInstancePointCloud(map,num);
