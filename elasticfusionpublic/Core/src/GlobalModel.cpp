   ...
   deformationNodes(NODE_TEXTURE_DIMENSION, 1, GL_LUMINANCE32F_ARB, GL_LUMINANCE, GL_FLOAT)
{
	...
	cudaGraphicsResourceGetMappedPointer((void**)&cuda_map_ptr, &num_bytes, mapCudaRes);//获取VBO的指针，返回给CUDA使用
							 ...
							 const FeedbackBuffer & filteredFeedback)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, rawFeedback.vbo);//VBO存放顶点数据，将rawFeedback.vbo绑定到GL_ARRAY_BUFFER
	//============================================== show ==========================================================
	//testFuction("1 rawFeedback.vbo");
	//============================================== show ==========================================================

	glEnableVertexAttribArray(0);//激活VAO的属性0，使得可以传递给shader
	glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, Vertex::SIZE, 0);//GL_ARRAY_BUFFER使用属性0

	glEnableVertexAttribArray(1);
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, Vertex::SIZE, reinterpret_cast<GLvoid*>(sizeof(Eigen::Vector4f)));

	glBindBuffer(GL_ARRAY_BUFFER, filteredFeedback.vbo);//将filteredFeedback.vbo绑定到GL_ARRAY_BUFFER
	...
	glEnable(GL_RASTERIZER_DISCARD);//关闭光栅化阶段
								   ...
								   const int timeDelta)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbos[target].first);//将vbos[target].first绑定到GL_ARRAY_BUFFER

	//============================================== show ==========================================================
	//testFuction("4 vbos[target].first");
	//============================================== show ==========================================================
	//告诉OpenGL，VAO中的哪个属性，使用哪个VBO
	//VAO记录属性，VBO管理顶点数据  
	glEnableVertexAttribArray(0); //enable VAO的属性0，使得可以传递给shader
	glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, Vertex::SIZE, 0); //编号为0的属性使用GL_ARRAY_BUFFER
...
{
	return cuda_map_ptr;//获取VBO的指针，返回给CUDA使用
}
