   ...
   dataType(dataType)
{
    if(cuda)
    {
        ...
        cudaGraphicsMapResources(1, &cudaRes, 0);//原本为cudaGraphicsMapResources(1, &cudaRes, 0); 作用为将opengl渲染的Texture对象，映射到cuda的显存地址中去
        ...
        cudaGraphicsUnmapResources(1, &cudaRes, 0);// 原本为cudaGraphicsUnmapResources(1, &cudaRes, 0);  //cuda处理完了即可解除资源锁定，OpenGL可以开始利用处理结果了。
